...
public class EnvioService {
    ...
    public EnvioDTO crearEnvio(EnvioDTO envioDTO) {
        ...
        Envio envio = envioRepository.save(envioDTO.toModel());
        return EnvioDTO.fromModel(envio);
    }

    public EnvioDTO getEnvioById(Long id) {
        Optional<Envio> envio = envioRepository.findById(id);
        return envio.map(EnvioDTO::fromModel).orElse(null);
    }

    public EnvioDTO actualizarEnvio(Long id, EnvioDTO envioActualizado) {
        ...
        if (envioExistente.isPresent()) {
            ...
            return EnvioDTO.fromModel(envioRepository.save(envio));
        }
    ...
    public List<EnvioDTO> getEnviosByMes(int mes) {
        return envioRepository.findByMes(mes).stream()
                .map(EnvioDTO::fromModel)
                .collect(java.util.stream.Collectors.toList());
    }

    public List<EnvioDTO> getEnviosByCorreo(String correo) {
        try {
            ...
            return envioRepository.findByIdUsuario(idUsuario).stream()
                    .map(EnvioDTO::fromModel)
                    .collect(java.util.stream.Collectors.toList());
        } catch (Exception e) {
            e.printStackTrace();
